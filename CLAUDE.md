...
2. **싱크 경계 압축 인코딩** (`ncEncFile`/`ncDecFile`/`ncDecodeSync`): IDB·메모리는 기존 객체 행 그대로 두고 **업로드 직전에만** 반복 문자열(상담원명·날짜·OB명·언어·연령대·오더일·유입일)을 파일 단위 사전으로 인터닝하고 행을 배열로 눕힌다. CTI의 `dur`은 nc가 안 쓰므로 제외(케어 통화시간은 tz가 자체 사본으로 계산). 인코딩은 업로드 루프가 **그룹 단위로 지연 호출**(`SLICE_ENCODE`) — 전체를 미리 인코딩하면 그것만으로 수백 MB.

실측: 192.3MB → **68.8MB(64% 감소)**, 그룹 7개, **최대 단일 문자열 11.8MB**(기존 192.3MB). 라운드트립 무손실 검증(표본 2만행 불일치 0, 재계산 모델의 DB이벤트·계약·콜·코디·5분류 전부 일치).

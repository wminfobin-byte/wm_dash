...
- **파서(`acParseReport`)**: 헤더에서 기간 추출(2자리 연도 → 2000+) → 라인 종류 4가지를 순서대로 판정. ① 누적 `-8월 누적 DB단가 …`(보고 월보다 큰 달 = 전년) ② 일자별 `-MM월 DD일(요일) …` ③ 패키지 `A. 더위크 : 광고비 …` ④ 매체 `GFA : 광고비 …`. **매체·패키지 목록은 하드코딩하지 않는다** — 원문에 있는 것만 잡고 새 패키지는 자동 편입, 없는 건 행 자체가 안 생긴다. `-`(값 없음)은 null로 보존해 출력에서 `-`로 나간다.
...
- **붙여넣기용 출력(GPT 대체 목적)**: `당일 버전`·`월 누적 버전` TSV 2종을 그대로 복사 → 엑셀 붙여넣기. 범위 = 최근 보고서 / 필터 기간 전체 토글, 옵션 = 광고비·DB수 열 포함 / 값 없는(-) 행 제외. 월 누적은 **보고서에 적힌 누적 단가**(그 달 것만, 일자 = 보고 마지막 날).

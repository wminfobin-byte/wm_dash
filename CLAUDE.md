...
- **화면 레이아웃**: 기간/빠른기간 필터 + KPI 4종 + 시간대별 계약률 차트(영어vs제2, Chart.js) + **전체/영어/제2외국어** 3섹션. 각 섹션 = ① `tbSlotTable` 시간대 요약표(slot×지표) + ② **`tbSlotDateBlocks` 시간대별 집행일 추이** — *시간대를 1차 기준*으로 그 시간대 집행일을 **시간순(오래된→최신)** 나열해 DB·계약 하락 추이를 봄(사용자 핵심 요구). 스타일은 `tbEnsureStyle`로 1회 주입.

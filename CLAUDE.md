...
- **언어별/일자별 매트릭스도 5단계**(`ncMatrixHeader`/`ncMatrixRow`, 45열): 전체3 + 5단계×(**시도DB수**·계약·계약률) + 군자1 + 5단계 콜×(시도·평균시도·연결·유효·유효율). 집계 버킷 `ncBlank`은 `db/c`=재콜레벨(rc 0~3), `cc/tr/co/va/td`=콜 5분류 인덱스(`NC_CATI`). 모수(`ncCatDb`)는 **당일·재콜·재재콜·회수콜 = 분배DB**(rc별), **이전(d1)만 시도DB수**( Set, `ncTdN`) — 이전은 당일과 같은 신규 코호트를 공유해 자기 분배 모수가 없기 때문. 헤더 라벨은 `ncCatDbLabel`이 갈라준다. `td`는 i===1일 때만 채운다(Set 최소화). 당일+재콜+재재콜+회수콜 분배DB 합 = 전체 DB수.

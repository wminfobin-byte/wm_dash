...
- **언어별/일자별 매트릭스도 5단계**(`ncMatrixHeader`/`ncMatrixRow`, 45열): 전체3 + 5단계×(**시도DB수**·계약·계약률) + 군자1 + 5단계 콜×(시도·평균시도·연결·유효·유효율). 집계 버킷 `ncBlank`은 `db/c`=재콜레벨(rc 0~3), `cc/tr/co/va/td`=콜 5분류 인덱스(`NC_CATI`). **모수는 분배DB가 아니라 시도DB**(`td` Set, `ncTdN`/`ncTdAll`) — 전체 열은 단계 간 중복을 제거해 단계별 합보다 작다. `td`는 매트릭스가 쓰는 **언어 버킷·일자 버킷에만** 채운다(개인별 표는 안 쓰므로 Set 생략 = 렌더 비용 절감).
...
- **UI**: KPI·섹션 헤더에 이모지 없음(사용자 요청). 표는 열마다 세로 구분선, 그룹 경계는 `nc-gs` 굵은 선.

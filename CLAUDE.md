...
- **언어별/일자별 매트릭스도 5단계**(`ncMatrixHeader`/`ncMatrixRow`, 45열): 전체3 + 5단계×(DB수·계약·계약률) + 군자1 + 5단계 콜×(시도·평균시도·연결·유효·유효율). 집계 버킷 `ncBlank`은 `db/c`=재콜레벨(rc 0~3), `cc/tr/co/va`=콜 5분류 인덱스(`NC_CATI`). **당일·이전은 같은 신규 DB 코호트가 모수**(`ncCatDb`)라 '이전' DB수는 회색 괄호 표기·합산 금지. 여기 **평균시도 = 시도 ÷ 분배DB**로 단계별 콜 섹션(시도DB 기준)과 정의가 달라 각주로 명시.

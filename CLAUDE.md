...
- **표시 형식 = DB수 | 계약수 | 계약률** (사용자 요청, 「전환율」이라는 말은 쓰지 않는다). 모든 표가 이 3칸 묶음(`pdTri`)으로 나온다.
- **핵심 지표 = 신규 DB 계약률(코호트)**: 분배일 1~N일 신규(비재콜·IN/콜백 제외) DB 중 분배 후 H일(기본 7, 당일/3/14 토글) 안에 신청된 비율. 신청일은 고객키 조인(일마감 우선·월간누적 보조). **월마다 같은 일자 범위(N = 데이터 마지막일 − H)**로 잘라 성숙도 차이를 없앤다. 계약 '건수'만 보면 DB가 늘어난 달이 좋아 보이므로(2026-09 실제 사례: 계약수 +21%, 신규 DB 계약률 −1.5%p) 반드시 이 계약률로 비교할 것.
- **하락 분해 (`pdDecompose`)**: 직전월 **패키지×매체그룹** 세그 계약률 × 기준월 물량 = 기대치. 기대−직전 = **믹스효과**(DB 구성 변화), 실제−기대 = **계약률 효과**(같은 세그 안에서 덜 팔림). 직전월 표본 < `PD_MIN_SEG`(30)인 세그는 '신규 세그'로 보고 믹스에 넣는다. 계열 기준 분해도 같이 표시(계열은 패키지 미스매치를 못 잡아 믹스가 과소 추정됨).
- **개인별 (`pdRenderPerson`)**: 기대 계약 모델은 쓰지 않는다(사용자 요청 — 단순 비교). 코디별로 **① 동기간**(이전 월들·기준월 모두 1~cohDay일 분배, DB수|계약수|계약률 + 기준월 계약률 증감 vs 각 월) / **② 월 전체 대비**(이전 월은 월 전체 — 단 분배일 ≤ 데이터 마지막일−H로 성숙분만 — vs 기준월 계약률 증감). 기준월 DB 50건 미만 제외, 정렬 기본값 = 직전월 동기간 대비 계약 손실(기준월 DB수×증감) 오름차순.
- **팀 귀속** = 최신 명단(`spBuildRoster`)의 담당. 팀 토글: 뇌새김 전체(영어+제2) / 영어 / 제2외국어 / 톡이즈.
- 섹션: 요약(자동 문장) → ① 월별 비교 → ② 하락 분해 카드 → ③ 차원별 표(`PD_DIMS`, 계약 손실 = 기준월 DB수 × 계약률 증감) → ④ 개인별 → ⑤ 주차별 추이(3일 내 계약, `PD_TREND_H`). 엑셀 = 섹션별 시트.
- **표 머리 클릭 정렬** (`pdSortTable`/`pdApplySorts`, 문서 레벨 click 위임): 모든 표 공통. 첫 클릭 = 숫자열 내림차순·글자열 오름차순, 재클릭 토글. `a→b` 셀은 b(기준월) 값, 빈 값/`-`는 항상 아래, `pd-diff` 증감행은 맨 아래 고정. 상태는 `컨테이너id#표순번`별로 기억해 필터 변경·재렌더 후에도 유지 — 그래서 **표는 마지막 헤더행 칸 = 본문 칸**(병합은 윗줄 그룹행에만)이어야 한다.
...
- **모수 = 일마감 ∪ 전체기간** (`src` 필드: `일마감` / `전체기간만` / `전체기간`). WM은 일마감만을 모수로 쓰지만 Via는 과거 구간의 일마감 파일이 아예 없을 수 있어, 그대로 두면 그 기간이 0건으로 보였다. **전체기간에만 있는 고객키도 일마감에 있었던 것으로 쳐서 모수에 넣는다.** 단 배송전취소는 일마감↔전체기간 대조로만 나오므로 이 건들은 **취소/유지만** 판정되고, 그 사실을 건수·신청일 범위와 함께 경고로 띄운다. 검증 원장의 `모수 출처` 열로 어느 쪽에서 왔는지 구분된다.
- **엑셀 파싱은 워크북의 모든 시트를 읽는다** (`vcParseWorkbook`). 첫 시트만 읽던 시절엔 월별 시트로 합쳐 올린 파일의 2번째 시트부터가 통째로 사라져, 그 고객키가 **어느 파일에도 없는 것처럼** 보였다. 시트마다 첫 행의 고객키 칸이 키처럼 안 보이면(`vcLooksKey`: 8자 이상·숫자 포함·한글 없음) 헤더로 보고 버리므로 헤더 없는 시트의 첫 데이터 행도 살아남는다. 파일 레코드에 `sheets`(시트명 목록)를 남겨 파일 관리에 `N행 · M시트`로 표시한다.
- **고객키 조회 (`vcLookupKeys`, 파일 관리 모달 상단)**: "이 계약이 왜 안 잡히지?"를 추측 없이 끝내는 진단 도구. 업로드된 원본 AOA를 직접 훑어 **어느 파일 몇 행**에 있는지 + 판정 컬럼(담당자 N·신청일 P·환급패키지 F·접수제품 U·반품일 AJ) 값 + 최종 판정(집계 포함 여부와 제외 사유)을 보여준다. 원본에 없으면 파일 누락·컬럼 위치·시트 문제로 좁혀 준다.
- **F열(환급패키지)은 어떤 판정에도 쓰이지 않는다** — `VC_APP` 상수와 싱크 컬럼 목록에만 존재(건기식 필터를 뺐을 때 소비처가 사라짐). 검증용으로 원장·고객키 조회에만 값을 노출한다.
- **접수제품 사전 (`vcProducts`: 원문 → `{label, include}`)** — 제품 목록을 코드에 박지 않는다:
  ...
  - **이름만 바뀐 동일 제품은 `VC_ALIAS`(원문 → 표시명)에 박아둔다** — 문자열 규칙으로 유도할 수 없기 때문. 현재: `캐리 EZ 삼륜 전기자전거` → `비아지오 삼륜 전기자전거_G3`, `전동 스쿠터_TYPE2` → `비아지오 전동 스쿠터_T2`(비아지오 표기가 정본). 매칭은 앞뒤 공백 제거 + 내부 공백 1칸 정규화, 구동방식 접미를 뗀 값으로도 한 번 더 조회한다.
  - **규칙(접미·별칭)을 고치면 `VC_LABEL_RULE_VER`를 올릴 것.** 이미 사전에 저장된 표시명은 자동 반영되지 않으므로 `vcLoadFiles`/`vcRestoreSync`가 `vcMigrateLabels()`로 1회 재정규화한다(그 뒤 사용자가 수동으로 나눈 건 보존).
...
- **화면**: 기간 필터+빠른기간 → **센터·코디 멀티선택** → **서브탭 `전체 | 제품A | 제품B …`**(접수제품 표시명에서 자동 생성, 데이터에 있는 것만) → KPI 5종 → AI 인사이트 → 월별 / 주차별 / 센터별 / **제품별**(WM의 '종류별' 자리) / 개인별.
- **다중 조합 조회**: 필터는 **기간 → 센터·코디 → 제품 서브탭** 순으로 AND 적용된다(빈 Set = 전체, nc 탭과 같은 관용구). 센터 선택을 바꾸면 코디 목록이 그 센터로 좁혀지고 기존 코디 선택은 초기화된다(화면에 안 보이는 필터가 남는 걸 막기 위해). 여기에 **개인별 표의 코디 `▶` 드릴다운**(`vcPersonDrill`)이 그 사람의 **제품 × 주차/월** 교차표를 펼친다 — 셀 = 취소율 · 취소수/최초계약수, 주차↔월 토글. `박숙자 × A모델 × 5월 4주차`를 필터 경로와 드릴 경로 양쪽으로 읽을 수 있다. **모수가 작아도 회색 처리나 숨김 없이 그대로 보여준다**(사용자 결정) — 대신 셀마다 모수를 같이 찍는다.
- **제품 색**: WM은 브랜드 4개가 고정이라 CSS에 박아뒀지만(`cd-brand-*`) Via 제품은 사전을 통해 늘어난다 → `productOrder` 인덱스로 12색 팔레트를 배정하고 `vcEnsureProdStyle`이 `<style id="vcProdColorStyle">`를 런타임 주입한다(그룹 헤더 = 진한 톤 `.bold-row`, 하위 월/주차 행 = 연한 톤, sticky 첫 열에도 같은 색). 색은 인덱스 고정이라 필터를 바꿔도 안 흔들린다. 서브탭·드릴표 제품명 앞 점도 같은 색.
- 개인별 표를 엑셀로 뽑을 땐 드릴다운 행(`.vc-drill-row`)이 중첩 표라 `table_to_sheet`가 어긋난다 → 복제본에서 걷어내고 변환한다.
- **검증 원장 CSV (`vcDownloadLedger`)**: 화면 숫자를 원본과 대조하기 위한 행 단위 덤프. 대시보드가 이미 두 파일을 고객키로 조인해뒀으므로 Excel에서는 조인 없이 **피벗 + VLOOKUP 샘플링**만으로 검증된다. 필터를 무시하고 전건을 내보내며, **집계에서 빠진 행도 사유와 함께**(`vcResult.skipped` = 일마감 반품 + 제외 제품) 담는다. 열 = 고객키·담당자 원문·코디명·센터·퇴사·신청일·반품일·접수제품 원문·표시명·상태·취소일수·전체기간매칭·집계포함·제외사유. **검산식: 원장 전체 행수 = 일마감 고객키(중복 제거) 수**, `집계포함=Y` 행수 = 전체 기간 '최초 계약수'. CSV는 BOM을 붙여 Excel 한글 깨짐을 막는다.
- **월별 취소율은 축이 둘이다**: ① 제품별(`vcMonthlyTable`) ② **매체/비매체별**(`vcMediaTable`, 나란히 별도 섹션 `vcNavMedia`). 같은 표를 `vcMonthlyHtml(data, g)`에 그룹 정의만 바꿔 넘겨 두 번 그린다(`g=null`이면 전체+월 행만). 매체 표는 제품 서브탭과 무관하게 항상 두 줄로 접힌다.
- **매체/비매체 판정 (`vcMediaOf`, Q열 분류)**: 공란이거나 `VC_NONMEDIA_WORDS`(`homepage_in`·`bike_c`·`youtuber`·`callback`·`tv_b`) 중 하나라도 **부분 문자열로** 들어 있으면 `비매체`, 나머지는 전부 `매체`. 대소문자 무시. **매체를 세분하지 않는다** — 새 비매체 코드가 생기면 이 배열에만 소문자로 추가한다. 색은 두 값 고정이라 CSS에 박아둔다(`.vc-media-y` 파랑 / `.vc-media-n` 회색).
  **전체 뷰의 월별·주차별은 제품별 그룹으로 접힌다**(WM이 구분=브랜드로 접던 자리를 제품이 대체). 제품 서브탭에서는 그 제품만 필터되고 월/주차 행만 나오며 제품별 섹션은 숨긴다.
- **센터 목록은 WM 상수를 쓰지 않는다.** 초기엔 `CD_CENTER_ORDER`를 그대로 물려받아 이름이 겹치는 `노원`·`군자`만 굵게(bold=true) 나오면서 총계 행과 구분이 안 됐다. 지금은 `vcCenterOrder(data)`가 **데이터에서 센터를 뽑아 계약수 내림차순**으로 정렬하고, 굵게 강조하는 행은 `전체` 하나뿐이다(센터별 표·개인별 정렬 양쪽에서 같은 순서를 쓴다).
...
- 다운로드 구간은 **바뀐 슬라이스만** 받으므로 보통 몇 초 만에 지나간다. 아래 「변경분만 받기」 참조.

## 동기화 다운로드 최소화 — 변경분만 받기 (2026-09-09)

예전에는 접속할 때마다 **압축 기준 약 100MB(nc 30 · tz 16 · tb 12 · sp 12 · gd 12 · cd 10 · ng 3 · vc 2)를 전부 받은 뒤에야** 최신인지 판정했다. 하루 2~4회 올라오는 동기화 때문에 낮 시간대 접속은 거의 항상 전량 다운로드였다. 두 단계로 줄였다.

### ① 매니페스트 선판정 (`fetchSyncCtx`)

`fetchSyncPayload`에서 매니페스트 조회 부분만 떼어냈다. 수 KB짜리 `sync_manifest.json`만 먼저 읽고 **기존과 똑같은 조건**으로 최신 여부를 본다.

- `payload.X`(다운로드 후 존재 여부) → `ctx.keys.has(x)`(매니페스트 슬라이스 존재 여부)로 대체. 업로더가 **값이 있는 키만** 올리므로 두 판정은 동치다.
- 최신이면 **바이트 하나 안 받고** 반환. 매니페스트가 없는 아주 구 포맷(`ctx.manifest===null`)이면 선판정을 건너뛰고 기존대로 전량 받는다.
- 판정이 틀려도 60초 폴러(`wmSyncPoll`)가 곧 잡는다.

### ② 슬라이스 단위 변경 감지 (`WM_SLICE_SIG_KEY`)

`ts`가 바뀌었다고 전 탭을 다시 받을 이유는 없다 — 광고비만 바뀌어도 100MB를 다시 받던 게 진짜 원인이었다.

- **업로드**: `zip.file('d', str, {date:new Date(0)})`로 압축을 **결정론적**으로 만들고(날짜가 안 들어가야 같은 내용 = 같은 바이트), 그룹별 GitHub blob sha를 `sliceMeta.shas`에 실어 매니페스트에 올린다. blob sha = 내용 SHA-1이므로 그대로 내용 해시로 쓴다.
- **다운로드**: `ctx.sig[key]`(그 키의 모든 그룹 sha를 이어붙인 서명) vs 로컬 `sync_slice_sig`. **서명이 같고 + (로컬에 데이터가 있거나 이 계정이 안 쓰는 탭)** 이면 그 슬라이스를 통째로 건너뛴다. `wantKeys`로 `fetchSyncPayload`에 넘긴다.
- **복원은 자동으로 걸러진다** — 기존 복원 분기가 전부 `if(payload.nc && …)` 꼴이라, 안 받은 슬라이스는 `undefined`가 되어 그냥 지나간다. 조건을 새로 손댈 필요가 없었다.
- `ALWAYS_KEYS`(`ls`·`ngResults`·`vcProducts`·`ac`, 합쳐서 수십 KB)는 항상 받아 예외 케이스를 없앤다.
- 서명 기록 시점: **사이클이 끝까지 성공했을 때만**(`sync_last_ts`와 같은 자리). 중간에 실패하면 서명이 안 남아 다음 접속에 다시 받는다 — 안전한 방향으로만 틀린다.
- **업로드한 PC**도 업로드 직후 `sliceMeta`로 서명을 기록해, 자기가 올린 데이터를 다음 접속에 도로 받지 않는다. 동기화 모달의 수동 전체 다운로드도 `fetchSyncCtx`를 먼저 불러 서명을 남긴다.
- **폴백이 전부 안전한 방향**이다: 서명 없는 구 매니페스트(`ctx.sig`가 빈 객체) → `wantKeys=null` → 전량 다운로드. 압축이 어쩌다 비결정론적이어도 서명이 매번 달라져 전량 다운로드가 될 뿐 데이터가 어긋나지는 않는다.
- **배포 직후 첫 접속은 여전히 전량 다운로드**다(로컬 서명이 비어 있음 + 압축 바이트가 date 고정으로 바뀜). 두 번째 접속부터 효과가 난다.

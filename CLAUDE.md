...
- **매체/비매체 판정 (`vcMediaOf`, Q열 분류)**: 공란이거나 `VC_NONMEDIA_WORDS`(`homepage_in`·`bike_c`·`youtuber`) 중 하나라도 **부분 문자열로** 들어 있으면 `비매체`, 나머지는 전부 `매체`. 대소문자 무시. **매체를 세분하지 않는다** — 새 비매체 코드가 생기면 이 배열에만 소문자로 추가한다. 색은 두 값 고정이라 CSS에 박아둔다(`.vc-media-y` 파랑 / `.vc-media-n` 회색).

...
- **개인별 표는 없다** (사용자 요청 — ③ 차원별 표의 `코디` 보기와 중복). 요약 문장용 `pdPersonInsights`만 남아 직전월 동기간 대비 계약 손실(기준월 DB수×계약률 증감) ≤ −8건 코디와 1인·1일당 물량 변화를 요약에 올린다.
- **팀 귀속** = 최신 명단(`spBuildRoster`)의 담당. 팀 토글: 뇌새김 전체(영어+제2) / 영어 / 제2외국어 / 톡이즈.
- 섹션: 요약(자동 문장) → ① 월별 비교 → ② 하락 분해 카드 → ③ 차원별 표(`PD_DIMS`, 계약 손실 = 기준월 DB수 × 계약률 증감) → ④ 주차별 추이(계약 인정기간 = 글로벌 필터 `pdH`, 분배일 ≤ 데이터 마지막일−H까지). 엑셀 = 섹션별 시트.
...
- **분석 섹션**: ① `ncRenderStage` 단계별 표(+일자별/코디별 × 단계 탭 — 맨 앞 **전체 합계** 탭(`ncStageCat='all'`, 기본값)은 행마다 `ncStageTotal`로 5단계를 합쳐 이전(d1) 분배DB 중복 제외) ② `ncRenderEff` 시도 효율·기회비용(차트 `ncEffChart`) ③ `ncRenderIdle` 미시도/저시도 DB(코디명 ▶ 클릭 시 `ncToggleIdle`로 코디×단계 분해 펼침). 이후 언어별 매트릭스·일자별·추이차트·개인별.
